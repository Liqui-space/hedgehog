...
contract Module3 is Ownable, FlashLoanReceiverBase {
    ...
    constructor() Ownable() public FlashLoanReceiverBase(ILendingPoolAddressesProvider(0xB53C1a33016B2DC2fF3653530bfF1848a515c8c5)) {
        ...
        IERC20(USDC).approve(address(LENDING_POOL), type(uint256).max);
        IERC20(OSQTH).approve(address(LENDING_POOL), type(uint256).max);
        IERC20(WETH).approve(address(LENDING_POOL), type(uint256).max);
    }
